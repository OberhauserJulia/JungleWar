...
@onready var PlayerColorSingelton = get_node("/root/NewColorsPlayer2")

var toggleInt := 1

func _ready():
	if PlayerColorSingelton.BodyColor:
		material.set_shader_parameter("newColorEyes", PlayerColorSingelton.BodyColor)
	if PlayerColorSingelton.EyeColor:
		material.set_shader_parameter("newColorBody", PlayerColorSingelton.EyeColor)
